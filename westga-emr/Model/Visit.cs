...
namespace westga_emr.Model
{
	...
	public class Visit
	{
		...
		/// <param name="symptoms_">Symptoms the patient exhibits during the Visit</param>
		/// <param name="finalDiagnosis_">The final diagnosis made during the Visit</param>
		public Visit( long? appointmentID_, int? nurseID_, DateTime visitDateTime_, string initialDiagnosis_, decimal weight_, int systolicPressure_, int diastolicPressure_, decimal bodyTemperature_, int pulse_, string symptoms_, string finalDiagnosis_)
		{
			if ( appointmentID_ < 1 || nurseID_ < 1)
			{
				throw new ArgumentException("id, appointmentID, and nurseID must be greater than zero");
			}
			if (visitDateTime_ == null)
